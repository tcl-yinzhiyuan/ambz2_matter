...
Cluster::Cluster(config_t config, uint32_t _cluster_id, uint16_t _endpoint_id, uint16_t cluster_flags)
{
    ...
    if (_cluster_id == Identify::Id)
    {
        if (flags & CLUSTER_FLAG_SERVER)
        {
            ...
            function_list[0] = (function_t)emberAfIdentifyClusterServerInitCallback;
            function_list[1] = (function_t)MatterIdentifyClusterServerAttributeChangedCallback;
            flags |= (CLUSTER_FLAG_INIT_FUNCTION | CLUSTER_FLAG_ATTRIBUTE_CHANGED_FUNCTION);
        }
    ...
    else if (_cluster_id == Groups::Id)
    {
        if (flags & CLUSTER_FLAG_SERVER)
        {
            ...
            function_list[0] = (function_t)emberAfGroupsClusterServerInitCallback;
            flags |= CLUSTER_FLAG_INIT_FUNCTION;
        }
        if (flags & CLUSTER_FLAG_CLIENT)
        {
    ...
    else if (_cluster_id == Scenes::Id)
    {
        if (flags & CLUSTER_FLAG_SERVER)
        {
            ...
            function_list[0] = (function_t)emberAfScenesClusterServerInitCallback;
            flags |= CLUSTER_FLAG_INIT_FUNCTION;
        }
        if (flags & CLUSTER_FLAG_CLIENT)
        {
    ...
    else if (_cluster_id == OnOff::Id)
    {
        if (flags & CLUSTER_FLAG_SERVER)
        {
            ...
            function_list[0] = (function_t)emberAfOnOffClusterServerInitCallback;
            flags |= CLUSTER_FLAG_INIT_FUNCTION;
        }
        if (flags & CLUSTER_FLAG_CLIENT)
        {
    ...
    else if (_cluster_id == LevelControl::Id)
    {
        if (flags & CLUSTER_FLAG_SERVER)
        {
            ...
            function_list[0] = (function_t)emberAfLevelControlClusterServerInitCallback;
            flags |= CLUSTER_FLAG_INIT_FUNCTION;
        }
        if (flags & CLUSTER_FLAG_CLIENT)
        {
    ...
    else if (_cluster_id == BasicInformation::Id)
    {
        if (flags & CLUSTER_FLAG_SERVER)
        {
            ...
            function_list[0] = (function_t)emberAfBasicInformationClusterServerInitCallback;
            flags |= CLUSTER_FLAG_INIT_FUNCTION;
        }
        if (flags & CLUSTER_FLAG_CLIENT)
        {
